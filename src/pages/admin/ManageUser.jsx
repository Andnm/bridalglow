import { Spin, Table, Button, Avatar, Menu, Dropdown, Modal } from "antd";
...
const ManageUser = () => {
  ...
  const handleUpdateRole = async (user) => {
    Modal.confirm({
      title: `Confirm Update Role for ${user.fullname}`,
      content: `Are you sure you want to upgrade the role of ${user.fullname} to Artist?`,
      okText: "Update",
      cancelText: "Cancel",
      okButtonProps: {
        style: {
      ...
      onOk: async () => {
        try {
          await updateArtistRoleByAdmin(user._id);
          const newOriginalData = originalData.filter(
            (item) => item._id !== user._id
          );
          const newProcessingData = processingData.filter(
            (item) => item._id !== user._id
          );
          setOriginalData(newOriginalData);
          setProcessingData(newProcessingData);
          ...
          <Menu>
            ...
            <Menu.Item key="banUnban">
              <Button
                type="link"
                onClick={() =>
                  handleBanUnban(record, record.status ? "ban" : "unban")
                }
            ...
            <Menu.Item key="updateRole">
              <Button
                type="link"
                onClick={() => handleUpdateRole(record)}
                icon={<FaPersonDotsFromLine style={{ fontSize: "20px" }} />}
                style={{ color: "#1890ff" }}
                className="flex items-center"
              >
                Update Artist Role
